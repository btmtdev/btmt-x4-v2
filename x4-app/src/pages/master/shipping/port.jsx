...
const CONFIG = {
  ...
  fields: [
    ...
    { key: "is_destination", label: "MASTER.IS_DESTINATION", type: "checkbox" },
    { key: "is_discharge", label: "MASTER.IS_DISCHARGE", type: "checkbox" },
  ],
}
const TITLE_KEY = "MASTER.TITLE_PORT"
                ...
                <div key={f.key} className="space-y-1">
                  ...
                  {f.type === "checkbox" ? (
                    <div className="flex items-center gap-2 h-9">
                      <input type="checkbox" className="size-4" checked={!!form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.checked }))} />
                    </div>
                  ) : f.type === "select" ? (
                    <select className="w-full border border-border rounded-md px-3 py-2 text-xs bg-background h-9" value={String(form[f.key] ?? "")} onChange={e => { const v = e.target.value; setForm(p => ({ ...p, [f.key]: v === "true" ? true : v === "false" ? false : v || null })) }}>
                      <option value="">-</option>
