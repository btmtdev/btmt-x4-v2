          ...
          <form onSubmit={save} className="flex flex-col h-full">
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              <div className="space-y-1">
                <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">KEY</label>
                <Input className="text-xs h-9 bg-muted/50" readOnly value={(form.customer_area || "") + (form.agech_code || "")} />
              </div>
              <div className="space-y-1">
                <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">NAME</label>
                <Input className="text-xs h-9" value={form.name ?? ""} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} />
              </div>
              <div className="space-y-1">
                <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">CUSTOMER</label>
                <select className="w-full border border-border rounded-md px-3 py-2 text-xs bg-background h-9" required value={form.customer_key ?? ""} onChange={e => setForm(p => ({ ...p, customer_key: e.target.value || null }))}>
                  <option value="">-</option>
              ...
              <div className="space-y-1">
                ...
                <select className="w-full border border-border rounded-md px-3 py-2 text-xs bg-background h-9" value={form.country_key ?? ""} onChange={e => setForm(p => ({ ...p, country_key: e.target.value || null }))}>
                  <option value="">-</option>
                  {(lookups.countries || []).map(o => <option key={o.key} value={o.key}>{o.key} - {o.name}</option>)}
                </select>
              </div>
              <div className="space-y-1">
                <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">CUSTOMER AREA</label>
                <Input className="text-xs h-9" value={form.customer_area ?? ""} onChange={e => setForm(p => ({ ...p, customer_area: e.target.value }))} />
              </div>
              <div className="space-y-1">
                <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">AGECH CODE</label>
                <Input className="text-xs h-9" value={form.agech_code ?? ""} onChange={e => setForm(p => ({ ...p, agech_code: e.target.value }))} />
              </div>
            </div>
