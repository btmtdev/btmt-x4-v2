...
const CONFIG = {
  ...
  fields: [
    ...
    { key: "so_type", label: "MASTER.SO_TYPE" },
    { key: "sap_id", label: "MASTER.SAP_ID", hideGrid: true },
    { key: "address1", label: "MASTER.ADDRESS1", hideGrid: true },
    { key: "address2", label: "MASTER.ADDRESS2", hideGrid: true },
    { key: "term_of_pay1", label: "MASTER.TERM_OF_PAY_1", hideGrid: true },
    { key: "term_of_pay2", label: "MASTER.TERM_OF_PAY_2", hideGrid: true },
    { key: "term_of_pay3", label: "MASTER.TERM_OF_PAY_3", hideGrid: true },
    { key: "trade_term", label: "MASTER.TRADE_TERM", hideGrid: true },
    { key: "ship_from", label: "MASTER.SHIP_FROM", hideGrid: true },
    { key: "edi_port", label: "MASTER.EDI_PORT", hideGrid: true },
  ],
}
const TITLE_KEY = "MASTER.TITLE_CUSTOMER"
...
export default function CustomerPage() {
  ...
  async function load() {
    setLoading(true)
    const data = await shippingService[CONFIG.service].getAll()
    setRows(data || [])
    setLoading(false)
  }
              ...
              {CONFIG.fields.map(f => (
                <div key={f.key} className="space-y-1">
                  ...
                  {f.type === "select" ? (
                    ...
                    <select className="w-full border border-border rounded-md px-3 py-2 text-xs bg-background h-9" value={form[f.key] ?? ""} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value === "" ? null : e.target.value }))}>
                      <option value="">-</option>
                      {(lookups[f.lookup] || []).map(o => <option key={o[f.valueKey]} value={o[f.valueKey]}>{o[f.valueKey]} - {o[f.displayKey]}</option>)}
                    </select>
                  ) : (
                    <Input className="text-xs h-9" type={f.type === "number" ? "number" : f.type === "date" ? "date" : "text"} step="any" required={f.required} value={form[f.key] ?? ""} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} />
                  )}
                </div>
              ))}
            </div>
