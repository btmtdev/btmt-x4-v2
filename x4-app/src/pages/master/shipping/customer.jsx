              ...
              {CONFIG.fields.map(f => (
                <div key={f.key} className="space-y-1">
                  ...
                  {f.type === "select" ? (
                    ...
                    <select className="w-full border border-border rounded-md px-3 py-2 text-sm bg-background h-9" value={form[f.key] ?? ""} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value === "" ? null : e.target.value }))}>
                      <option value="">-</option>
                      {(lookups[f.lookup] || []).map(o => <option key={o[f.valueKey]} value={o[f.valueKey]}>{o[f.valueKey]} - {o[f.displayKey]}</option>)}
                    </select>
                  ) : (
                    <Input className="text-sm h-9" type={f.type === "number" ? "number" : f.type === "date" ? "date" : "text"} step="any" required={f.required} value={form[f.key] ?? ""} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} />
                  )}
                </div>
              ))}
            </div>
