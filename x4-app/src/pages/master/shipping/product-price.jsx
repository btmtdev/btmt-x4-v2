...
const CONFIG = {
  ...
  idField: "id",
  fields: [
    { key: "product_key", label: "MASTER.PRODUCT_KEY", type: "lookup", lookup: "products", valueKey: "key", displayKey: "description", required: true },
    { key: "so_type", label: "MASTER.SO_TYPE", type: "select", options: ["A", "B"] },
    { key: "price", label: "MASTER.PRICE", type: "number", required: true },
    { key: "effective_from", label: "MASTER.EFFECTIVE_FROM", type: "date" },
    { key: "effective_to", label: "MASTER.EFFECTIVE_TO", type: "date" },
  ],
}
const TITLE_KEY = "MASTER.TITLE_PRODUCT_PRICE"
...
export default function ProductPricePage() {
  ...
  function changeDate(val) {
    setFilterDate(val)
    if (!val) { setRows(allRows); return }
    setRows(allRows.filter(r => r.effective_from <= val && r.effective_to >= val))
  }
  ...
  function openAdd() {
    const defaults = {}
    if (periods.length) {
      defaults.effective_from = periods[0].effective_from
      defaults.effective_to = periods[0].effective_to
    }
    setForm(defaults); setPanel("add"); loadLookups()
  }
    ...
    <div className="h-full flex flex-col p-6 gap-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <input type="date" className="border border-border rounded-md px-2 py-1 text-xs bg-background h-8" value={filterDate} onChange={e => changeDate(e.target.value)} />
        </div>
      </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              {CONFIG.fields.map(f => (
                <div key={f.key} className="space-y-1">
                  <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{f.label.split(".").pop().replace(/_/g, " ").toUpperCase()}</label>
                  {f.type === "select" ? (
                    <select className="w-full border border-border rounded-md px-3 py-2 text-xs bg-background h-9" value={form[f.key] ?? ""} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))}>
                      <option value="">-</option>
                      {f.options.map(o => <option key={o} value={o}>{o}</option>)}
                    </select>
                  ) : f.type === "lookup" ? (
                    <select className="w-full border border-border rounded-md px-3 py-2 text-xs bg-background h-9" value={form[f.key] ?? ""} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value === "" ? null : isNaN(e.target.value) ? e.target.value : Number(e.target.value) }))}>
                      <option value="">-</option>
                      {(lookups[f.lookup] || []).map(o => <option key={o[f.valueKey]} value={o[f.valueKey]}>{o[f.valueKey]} - {o[f.displayKey]}</option>)}
                    </select>
                  ) : (
                    <Input className="text-xs h-9" type={f.type === "number" ? "number" : f.type === "date" ? "date" : "text"} step="any" required={f.required} value={form[f.key] ?? ""} onChange={e => setForm(p => ({ ...p, [f.key]: f.type === "number" ? (e.target.value === "" ? null : Number(e.target.value)) : e.target.value }))} />
                  )}
                </div>
              ))}
            </div>
