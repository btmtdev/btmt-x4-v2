...
export default function SalesOrderPage() {
  ...
  async function openPanel(row) {
    ...
    const invRes = await fetch(`${getShippingApiUrl()}/api/invoices?so_key=${encodeURIComponent(row.so_key)}&so_line=${encodeURIComponent(row.so_line)}`).then(r => r.json()).then(d => d.data).catch(() => [])
    ...
    <div className="h-full flex flex-col p-6 gap-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-sm">
          <select className="h-8 border rounded px-2 bg-background text-sm" value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
            <option value="open">{t("SO.OPEN")}</option>
            <option value="closed">{t("SO.CLOSED")}</option>
            <option value="all">{t("ALL")}</option>
          </select>
          <select className="h-8 border rounded px-2 bg-background text-sm" value={filterPeriod} onChange={e => setFilterPeriod(e.target.value)}>
            <option value="">{t("SO.ALL_PERIODS")}</option>
            {periods.map(p => <option key={p} value={p}>{p}</option>)}
          </select>
          <Input className="h-8 w-56 text-sm" placeholder={t("SO.SEARCH_PLACEHOLDER")} value={search} onChange={e => setSearch(e.target.value)} />
          <Button size="sm" className="text-sm" onClick={() => document.getElementById("so-file-input").click()}>{t("SO.IMPORT")}</Button>
        ...
        <SlidePanel width={panelWidth} onClose={closePanel} onResize={setPanelWidth} maxWidth={90} title={`${form.so_key} / ${form.so_line}`}>
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="flex border-b">
              <button onClick={() => setPanelTab("detail")} className={`px-4 py-2 text-sm font-medium border-b-2 ${panelTab === "detail" ? "border-primary text-primary" : "border-transparent text-muted-foreground"}`}>{t("SO.TAB_DETAIL")}</button>
              <button onClick={() => setPanelTab("invoices")} className={`px-4 py-2 text-sm font-medium border-b-2 ${panelTab === "invoices" ? "border-primary text-primary" : "border-transparent text-muted-foreground"}`}>{t("SO.TAB_INVOICES")} ({soInvoices.length})</button>
            </div>

            {panelTab === "detail" && (<>
            {/* Revision Table */}
            <div className="overflow-x-auto border-b p-4">
              <h4 className="text-sm font-semibold mb-2">{t("SO.HISTORY")}</h4>
              {revisions.length === 0 ? (
                <p className="text-sm text-muted-foreground py-2">{t("SO.NO_REVISIONS")}</p>
              ) : (
            ...
            <div className="p-5 space-y-3">
              <h4 className="text-sm font-semibold">{t("MASTER.EDIT")}</h4>
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1 col-span-2">
                  <label className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">QTY CONFIRMED</label>
                  <Input className="text-sm h-9" type="number" value={form.qty_confirmed ?? 0} onChange={e => setForm(p => ({ ...p, qty_confirmed: Number(e.target.value) }))} />
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">PRODUCTION PERIOD</label>
                  <Input className="text-sm h-9" value={form.production_period ?? ""} onChange={e => setForm(p => ({ ...p, production_period: e.target.value }))} />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  <label className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">STOCK TYPE</label>
                  <Input className="text-sm h-9" value={form.stock_type ?? ""} onChange={e => setForm(p => ({ ...p, stock_type: e.target.value }))} />
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">REQ1</label>
                  <Input className="text-sm h-9" value={form.req1 ?? ""} onChange={e => setForm(p => ({ ...p, req1: e.target.value }))} />
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">REQ2</label>
                  <Input className="text-sm h-9" value={form.req2 ?? ""} onChange={e => setForm(p => ({ ...p, req2: e.target.value }))} />
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">REMARK</label>
                <Input className="text-sm h-9" value={form.remark ?? ""} onChange={e => setForm(p => ({ ...p, remark: e.target.value }))} />
              </div>
            </div>
            </>)}
            ...
            {panelTab === "invoices" && (
              <div className="p-4">
                {soInvoices.length === 0 ? (
                  <p className="text-sm text-muted-foreground">{t("SO.NO_INVOICES")}</p>
                ) : (
