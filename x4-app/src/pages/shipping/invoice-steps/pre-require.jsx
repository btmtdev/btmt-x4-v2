    ...
    <div className="p-4 space-y-4">
      <div className="space-y-4">
        <div className="space-y-1">
          ...
          <select className="w-full h-8 border rounded px-2 text-xs bg-background" value={form.customer_key ?? ""} onChange={e => {
        ...
        <div className="space-y-1">
          ...
          <select className="w-full h-8 border rounded px-2 text-xs bg-background" value={form._countryFilter ?? ""} onChange={e => setForm(p => ({ ...p, _countryFilter: e.target.value, port_of_destination: "", port_of_discharge: "" }))} disabled={!form.customer_key}>
            <option value="">{t("SUPPORT.SELECT")}</option>
        ...
        <div className="space-y-1">
          ...
          <select className="w-full h-8 border rounded px-2 text-xs bg-background" value={form.port_of_discharge ?? ""} onChange={e => { const val = e.target.value; const port = ports.find(p => (p.country_key || "") + (p.key_ || p.key) === val); setForm(p => ({ ...p, port_of_discharge: val, edi_ship_to: port?.name || "" })) }} disabled={!form._countryFilter}>
            <option value="">{t("SUPPORT.SELECT")}</option>
            {(() => {
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-semibold uppercase text-muted-foreground">ETD</label>
          <Input className="text-xs h-8" type="date" value={form.etd ?? ""} onChange={e => { const etd = e.target.value; const d = new Date(etd); d.setDate(d.getDate() - 1); setForm(p => ({ ...p, etd, schedule: d.toISOString().slice(0, 10) })) }} />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-semibold uppercase text-muted-foreground">INVOICE DATE</label>
          <Input className="text-xs h-8" type="date" value={form.invoice_date ?? ""} onChange={e => setForm(p => ({ ...p, invoice_date: e.target.value }))} />
        </div>
      </div>
    </div>
  )
