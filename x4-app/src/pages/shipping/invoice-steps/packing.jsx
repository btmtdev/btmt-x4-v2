  ...
  return (
    <div className="flex flex-col gap-4 p-4 h-full overflow-auto">
      ...
      <div className="flex items-center gap-2 flex-wrap border-b pb-3">
        <select className="h-8 border rounded px-2 text-xs bg-background" value={containerType} onChange={e => setContainerType(e.target.value)}>
        ...
        <input className="h-8 w-14 border rounded px-2 text-xs text-center" type="number" min={1} value={containerCount} onChange={e => setContainerCount(Math.max(1, Number(e.target.value)))} />
        <Button size="sm" variant="outline" className="text-xs gap-1" onClick={addContainers} disabled={!containerType}><Plus size={14} /> {t("DOCFLOW.PACK_ADD")}</Button>
        <div className="w-px h-6 bg-border mx-1" />
        <Button size="sm" className="text-xs gap-1" onClick={autoGen} disabled={!containerType}><Wand2 size={14} /> {t("DOCFLOW.PACK_AUTO")}</Button>
        {revision === 0 && packing.length > 0 && <Button size="sm" variant="outline" className="text-xs gap-1" onClick={reSeqAll}><ListOrdered size={14} /> {t("DOCFLOW.PACK_RESEQ")}</Button>}
        {packing.length > 0 && <Button size="sm" variant="ghost" className="text-xs text-red-500" onClick={() => { setPackingItems(prev => prev.filter(pi => lockedItems.has(`${pi.seq_no}|${pi.so_key}|${pi.so_line}`))) }}>{t("DOCFLOW.PACK_CLEAR")}</Button>}
      </div>

      {/* Unpacked summary */}
      {unpackedItems.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded p-2">
          <p className="text-[11px] font-semibold text-amber-700 mb-1">{t("DOCFLOW.PACK_UNPACKED")} ({unpackedItems.length})</p>
          <div className="flex flex-wrap gap-1">
            {unpackedItems.map(it => (
              <span key={`${it.so_key}|${it.so_line}`} className="text-[10px] bg-white border rounded px-1.5 py-0.5">{it.product_key}/{it.kit_key} <span className="text-muted-foreground">×{it.remaining}</span></span>
            ))}
          </div>
        </div>
      )}
        ...
        <div key={cont.seq_no} className="border rounded">
          <div className="flex items-center justify-between px-3 py-2 bg-muted/30 border-b">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold">#{cont.seq_no}</span>
              <span className="text-[10px] px-1.5 py-0.5 bg-background border rounded">{cont.container_type || "—"}</span>
              <span className="text-[10px] text-muted-foreground">{cont.total_qty} pcs</span>
              <span className="text-[10px] text-muted-foreground">|</span>
              <span className="text-[10px]">{cont.total_vol.toFixed(2)} / {cont.max_vol} m³</span>
              ...
              <span className="text-[10px] font-medium">{cont.pct}%</span>
            </div>
            <div className="flex items-center gap-1">
              {unpackedItems.length > 0 && (
                <select className="h-6 border rounded px-1 text-[10px] bg-background" value="" onChange={e => { if (e.target.value) { const it = unpackedItems.find(x => `${x.so_key}|${x.so_line}` === e.target.value); if (it) addToContainer(cont.seq_no, it); e.target.value = "" } }}>
          ...
          {cont.items.length > 0 ? (
              ...
              <tbody>
                {cont.items.map((pi, i) => {
                  ...
                  <tr key={i} className={`border-b border-border/20 hover:bg-muted/20 ${isLocked ? "bg-green-50" : ""}`}>
                    ...
                    <td className="py-0.5 text-right">{isLocked ? <span className="text-xs pr-1">{pi.qty}</span> : <input type="number" className="w-16 border rounded px-1 text-right text-[11px] h-5" value={pi.qty} onChange={e => updateItemQty(cont.seq_no, pi.so_key, pi.so_line, e.target.value)} />}</td>
            ...
            <p className="text-[11px] text-muted-foreground p-3 italic">{t("DOCFLOW.PACK_EMPTY_CONTAINER")}</p>
          )}
        </div>
      )})}
      ...
      {grouped.length === 0 && (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-xs">{t("DOCFLOW.PACK_EMPTY_HINT")}</p>
          </div>
        </div>
      )}
    </div>
