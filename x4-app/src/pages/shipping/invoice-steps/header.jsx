...
export default function InvoiceHeader({ form, setForm, ports, portsOfLoad, carriers, items, productBrands = {}, components = [], t }) {
  ...
  const f = (key, label, type = "text", disabled = false) => (
    <div className="space-y-1" key={key}>
      <label className="text-sm font-semibold uppercase text-muted-foreground">{label}</label>
      <Input className="text-sm h-8" type={type} value={form[key] ?? ""} disabled={disabled} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} />
    </div>
  )
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex gap-1 border-b">
        {tabs.map(tb => (
          <button key={tb.key} onClick={() => setHeaderTab(tb.key)} className={`px-3 py-1.5 text-sm font-semibold border-b-2 ${headerTab === tb.key ? "border-primary text-primary" : "border-transparent text-muted-foreground"}`}>{tb.label}</button>
        ))}
      </div>
      {headerTab === "shipping" && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1" key="port_of_discharge">
              ...
              <select className="w-full h-8 border rounded px-2 text-sm bg-background" value={form.port_of_discharge ?? ""} onChange={e => { const val = e.target.value; const port = ports.find(p => (p.country_key || "") + (p.key_ || p.key) === val); setForm(p => ({ ...p, port_of_discharge: val, edi_ship_to: port?.name || "" })) }}>
          ...
          <div className="grid grid-cols-5 gap-3">
            <div className="space-y-1 col-span-2" key="feeder_vessel">
              <label className="text-sm font-semibold uppercase text-muted-foreground">Feeder Vessel</label>
              <Input className="text-sm h-8" value={form.feeder_vessel ?? ""} onChange={e => setForm(p => ({ ...p, feeder_vessel: e.target.value }))} />
            </div>
            <div className="space-y-1 col-span-2" key="mother_vessel">
              <label className="text-sm font-semibold uppercase text-muted-foreground">Mother Vessel</label>
              <Input className="text-sm h-8" value={form.mother_vessel ?? ""} onChange={e => setForm(p => ({ ...p, mother_vessel: e.target.value }))} />
            </div>
            {f("voyage_no", "Voyage No")}
          </div>
          <div className="grid grid-cols-5 gap-3">
            <div className="space-y-1 col-span-2" key="booking_no">
              <label className="text-sm font-semibold uppercase text-muted-foreground">Booking No</label>
              <Input className="text-sm h-8" value={form.booking_no ?? ""} onChange={e => setForm(p => ({ ...p, booking_no: e.target.value }))} />
            </div>
            <div className="space-y-1 col-span-2" key="bl_no">
              <label className="text-sm font-semibold uppercase text-muted-foreground">B/L No</label>
              <Input className="text-sm h-8" value={form.bl_no ?? ""} onChange={e => setForm(p => ({ ...p, bl_no: e.target.value }))} />
            </div>
            {f("confirm_date", "Confirm Date", "date")}
          </div>
          <div className="grid grid-cols-5 gap-3">
            <div className="space-y-1 col-span-2" key="contract_no">
              <label className="text-sm font-semibold uppercase text-muted-foreground">Contract No</label>
              <Input className="text-sm h-8" value={form.contract_no ?? ""} onChange={e => setForm(p => ({ ...p, contract_no: e.target.value }))} />
            </div>
            <div className="space-y-1 col-span-2" key="lc_no">
              <label className="text-sm font-semibold uppercase text-muted-foreground">L/C No</label>
              <Input className="text-sm h-8" value={form.lc_no ?? ""} onChange={e => setForm(p => ({ ...p, lc_no: e.target.value }))} />
            </div>
            {f("lc_date", "L/C Date", "date")}
          </div>
          <div className="space-y-2">
            <div className="space-y-1" key="desc_of_goods1">
              <label className="text-sm font-semibold uppercase text-muted-foreground">Desc of Goods 1</label>
              <div className="flex gap-1">
                <Input className="text-sm h-8 flex-1" value={form.desc_of_goods1 ?? ""} onChange={e => setForm(p => ({ ...p, desc_of_goods1: e.target.value }))} />
