...
function StatusBadge({ status }) {
  return <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-semibold uppercase ${STATUS_COLORS[status] || "bg-gray-100"}`}>{status}</span>
}
...
export default function InvoicesPage() {
  ...
  async function load() {
    setLoading(true)
    const res = await fetch(`${import.meta.env.VITE_SHIPPING_API_URL ?? "http://localhost:4001"}/api/invoices?month=${filterMonth}`).then(r => r.json()).then(d => d.data) || []
    setData(res)
    setLoading(false)
  }
    ...
    <div className="h-full flex flex-col p-6 gap-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-xs">
          <select className="h-8 border rounded px-2 bg-background text-xs" value={filterMonth.split("-")[0]} onChange={e => setFilterMonth(e.target.value + "-" + filterMonth.split("-")[1])}>
            {Array.from({length: 5}, (_, i) => 2024 + i).map(y => <option key={y} value={y}>{y}</option>)}
          </select>
          <select className="h-8 border rounded px-2 bg-background text-xs" value={filterMonth.split("-")[1]} onChange={e => setFilterMonth(filterMonth.split("-")[0] + "-" + e.target.value)}>
            {Array.from({length: 12}, (_, i) => String(i+1).padStart(2,"0")).map(m => <option key={m} value={m}>{m}</option>)}
          </select>
          <Input className="h-8 w-56 text-xs" placeholder={t("DOCFLOW.IV_SEARCH")} value={search} onChange={e => setSearch(e.target.value)} />
          <Button size="sm" className="text-xs" onClick={() => { setPanel("create"); setInvoice(null) }}>{t("DOCFLOW.IV_CREATE")}</Button>
        </div>
      </div>
