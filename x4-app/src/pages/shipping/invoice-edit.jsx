...
export default function InvoiceEditPage({ onClose, invoiceKey: invoiceKeyProp }) {
  ...
  async function loadLookups() {
    const [p, ca, co, ct, dest, ctr, pol, prods] = await Promise.all([
      ...
      fetch(`${import.meta.env.VITE_SHIPPING_API_URL ?? "http://localhost:4001"}/api/masters/ports-of-load`).then(r => r.json()).then(d => d.data),
      fetch(`${import.meta.env.VITE_SHIPPING_API_URL ?? "http://localhost:4001"}/api/masters/products`).then(r => r.json()).then(d => d.data),
    ])
  ...
  async function loadInvoice() {
    setLoading(true)
    const res = await fetch(`${import.meta.env.VITE_SHIPPING_API_URL ?? "http://localhost:4001"}/api/invoices/${encodeURIComponent(invoiceKey)}`).then(r => r.json()).then(d => d.data)
    if (res?.invoice) {
      setForm(res.invoice)
  ...
  async function handleSave() {
    ...
    const url = `${import.meta.env.VITE_SHIPPING_API_URL ?? "http://localhost:4001"}/api/invoices${isNew ? "" : `/${encodeURIComponent(key)}`}`
    ...
    if (packing.length > 0) {
      ...
      await fetch(`${import.meta.env.VITE_SHIPPING_API_URL ?? "http://localhost:4001"}/api/invoices/${encodeURIComponent(key)}/packing`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(packPayload) })
    }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex border-b">
        {STEP_KEYS_EDIT.map((s, i) => (
          <button key={i} onClick={() => setStep(i)} className={`px-4 py-2.5 text-xs font-medium border-b-2 ${i === step ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
            {t(`DOCFLOW.${s}`)}
          </button>
        ))}
      </div>
