...
export default function TicketsPage() {
  ...
  function renderComment(text) {
    ...
    return parts.map((part, i) => {
      ...
      if (m) { const url = m[1]; if (/\.(jpg|jpeg|png|gif|webp|svg)$/i.test(url)) return <img key={i} src={url} alt="" className="max-w-[180px] rounded mt-1 cursor-pointer" onClick={() => window.open(url, "_blank")} />; return <a key={i} href={url} target="_blank" rel="noopener" className="text-primary hover:underline text-xs">{url.split("/").pop()}</a> }
    ...
    <div className="h-full flex overflow-hidden">
      ...
      <div className="w-[380px] shrink-0 border-r border-border flex flex-col bg-background">
        ...
        <div className="p-3 space-y-2 border-b border-border">
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("MASTER.SEARCH")} className="h-8 text-xs" />
          <select value={filter} onChange={e => setFilter(e.target.value)} className="w-full h-8 text-xs border border-input rounded-md px-2 bg-background">
          ...
          {filtered.map(t2 => (
            ...
            >
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="text-[11px] font-mono text-muted-foreground">{t2.key}</span>
              ...
              <p className="text-xs leading-snug line-clamp-1 pl-4">{t2.topic}</p>
              <p className="text-[10px] text-muted-foreground pl-4 mt-0.5">{t2.created_by} · {timeAgo(t2.updated_at || t2.created_at, i18n.language, t)}</p>
            </div>
          ))}
          {!filtered.length && <p className="text-xs text-muted-foreground text-center py-10">{t("SUPPORT.NO_ITEMS")}</p>}
        </div>
      </div>
          ...
          <>
            ...
            <div className="px-6 py-4 bg-background border-b border-border space-y-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs text-muted-foreground">{detail.ticket?.created_by}</span>
                <span className="text-xs text-muted-foreground">· {new Date(detail.ticket?.created_at).toLocaleDateString(i18n.language === "th" ? "th-TH" : "en-US", { day: "numeric", month: "short", year: "numeric" })}</span>
                {getPermPath(detail.ticket?.path) && <a href={getPermPath(detail.ticket?.path).path} target="_blank" rel="noopener" className="text-xs text-primary hover:underline ml-1">{getPermPath(detail.ticket?.path).name}</a>}
              </div>

              {isAdmin && (
                <div className="flex gap-2">
                  <select value={detail.ticket?.status || "open"} onChange={e => changeStatus(detail.ticket.key, e.target.value)} className="text-xs border border-input rounded-md px-2 py-1 bg-background">
                    {STATUSES.map(s => <option key={s} value={s}>{t(STATUS_KEY[s])}</option>)}
                  </select>
                  <select value={detail.ticket?.category || ""} onChange={e => { detail.ticket.category = e.target.value; setDetail({ ...detail }); req(`/api/tickets/${detail.ticket.key}`, { method: "PUT", body: JSON.stringify({ category: e.target.value }) }).then(load) }} className="text-xs border border-input rounded-md px-2 py-1 bg-background">
                    {CATEGORIES.map(c => <option key={c} value={c}>{t(CAT_KEY[c])}</option>)}
                  </select>
                  <select value={detail.ticket?.priority || "medium"} onChange={e => { detail.ticket.priority = e.target.value; setDetail({ ...detail }); req(`/api/tickets/${detail.ticket.key}`, { method: "PUT", body: JSON.stringify({ priority: e.target.value }) }).then(load) }} className="text-xs border border-input rounded-md px-2 py-1 bg-background">
            ...
            {detail.ticket?.description && (
              <div className="px-6 py-4 bg-background border-b border-border">
                ...
                {(() => {
                  ...
                  return f.length ? <div className="flex flex-wrap gap-2 mt-2">{f.map((url, i) => <a key={i} href={url} target="_blank" rel="noopener" className="text-xs text-primary hover:underline">{url.split("/").pop()}</a>)}</div> : null
                })()}
              </div>
            )}
              ...
              {detail.history?.map(h => {
                ...
                return (
                  <div key={h.id} className="group">
                    ...
                    {isComment && h.comment && (
                      <div className="mt-1 text-xs bg-background rounded-md px-3 py-2 border border-border leading-relaxed">{renderComment(h.comment)}</div>
                    )}
                  </div>
                )
              })}
            ...
            <div className="px-6 py-3 bg-background border-t border-border space-y-1.5 shrink-0">
              {commentFiles.length > 0 && <div className="flex flex-wrap gap-1">{commentFiles.map((url, i) => <span key={i} className="text-xs bg-muted px-2 py-0.5 rounded">{url.split("/").pop()} <button onClick={() => setCommentFiles(f => f.filter((_, j) => j !== i))} className="hover:text-destructive">&times;</button></span>)}</div>}
              {uploading && <p className="text-xs text-primary animate-pulse">{t("SUPPORT.UPLOADING")}</p>}
              <div className="flex gap-2">
                <Input placeholder={t("SUPPORT.ADD_COMMENT")} value={comment} onChange={e => setComment(e.target.value)} onKeyDown={e => e.key === "Enter" && addComment()} className="flex-1 h-8 text-xs" />
                <label className="inline-flex items-center h-8 px-2 border border-input rounded-md cursor-pointer hover:bg-accent text-xs text-muted-foreground shrink-0">
