      ...
      <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
          <div className="space-y-2">
            ...
            {errors.subject && <p className="text-xs text-destructive">{t("SUPPORT.REQUIRED")}</p>}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {CATEGORIES.map(c => (
                <button key={c} type="button" onClick={() => setForm(f => ({ ...f, category: c }))} className={`rounded-md border px-3 py-2 text-xs transition-colors ${form.category === c ? "border-primary bg-primary/5 text-primary font-medium" : "border-border text-muted-foreground hover:bg-accent"}`}>{t(CAT_KEY[c])}</button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-1.5">
              {PRIORITIES.map(p => (
                <button key={p} type="button" onClick={() => setForm(f => ({ ...f, priority: p }))} className={`flex-1 rounded-md border px-2 py-1.5 text-xs transition-colors ${form.priority === p ? "border-primary bg-primary/5 text-primary font-medium" : "border-border text-muted-foreground hover:bg-accent"}`}>{t(PRI_KEY[p])}</button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} className="flex w-full rounded-md border border-input bg-background px-3 py-2 text-xs ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 min-h-[100px] resize-y" placeholder={t("SUPPORT.DESCRIPTION_PLACEHOLDER")} />
          </div>

          <div className="space-y-2">
            <Label>{t("SUPPORT.RELATED_MENU")}</Label>
            <select value={form.permission_id} onChange={e => setForm(f => ({ ...f, permission_id: e.target.value || "" }))} className="flex h-9 w-full rounded-md border border-input bg-background px-3 text-xs">
              <option value="">—</option>
        ...
        <div className="px-5 py-3 space-y-1.5 shrink-0">
          {files.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {files.map((url, i) => (
                <span key={i} className="text-xs bg-muted px-2 py-0.5 rounded">{url.split("/").pop()} <button type="button" onClick={() => setFiles(f => f.filter((_, j) => j !== i))} className="hover:text-destructive">&times;</button></span>
              ))}
            </div>
          )}
