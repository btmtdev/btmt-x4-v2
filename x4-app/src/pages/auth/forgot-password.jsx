    ...
    <div className="flex items-center gap-2 mb-6">
      {Array.from({ length: total }, (_, i) => (
        <div key={i} className="flex items-center gap-2 flex-1">
          <div
            className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${i + 1 <= current ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
            {i + 1}
          </div>
          {i < total - 1 && (
    ...
    <div className="space-y-4">
      ...
      {step === 2 && (
        <form onSubmit={handleVerifyMobile} className="space-y-3">
          <p className="text-xs text-muted-foreground">{t("MOBILE_HINT")}</p>
          <MobileInput
