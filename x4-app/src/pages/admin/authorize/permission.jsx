      ...
      {reparent && (
        <Dialog open onOpenChange={v => !v && setReparent(null)}>
          <DialogContent className="max-w-sm">
            ...
            <div>
              <label className="text-sm font-medium">{t("PERMISSIONS.NEW_PARENT")}</label>
              <select className="mt-1 w-full h-9 rounded border text-sm px-2" value={newParent} onChange={e => setNewParent(e.target.value)}>
    ...
    <>
      <div className="flex items-center py-1 px-3 hover:bg-muted/30 group">
        <div className="flex items-center gap-2 w-[400px] shrink-0" style={{ paddingLeft: depth * 24 }}>
          ...
          <span className="text-sm font-medium truncate">{node[nameKey]}</span>
        </div>
        <span className="text-sm text-muted-foreground flex-1 truncate">{node.path || ""}</span>
