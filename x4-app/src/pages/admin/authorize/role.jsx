...
export default function RoleManagement() {
  ...
  async function openUserPanel(role) {
    ...
    fetch(`${getApiUrl()}/api/settings/avatars`, {
      method: "POST",
              ...
              <div className="grid grid-cols-3 gap-3 py-2">
                {permissions.filter(p => p.is_authorized !== false).map(p1 => (
                  <div key={p1.key} className="border border-border rounded-lg overflow-hidden">
                    <label className="flex items-center gap-3 px-3 py-2 bg-muted/50 border-b border-border cursor-pointer font-medium text-sm">
        ...
        <Dialog open onOpenChange={v => !v && setSelectedRole(null)}>
          <DialogContent className="max-w-md max-h-[80vh] flex flex-col">
              ...
              {addSearch && !addUserId && (
                <div className="absolute z-10 top-full left-0 right-12 mt-1 bg-background border border-border rounded-lg shadow-lg max-h-48 overflow-auto">
                  {allUsers.filter(u => !roleUsers.some(ru => ru.key === u.key) && (u.key.includes(addSearch.toLowerCase()) || (u.display_name_en || "").toLowerCase().includes(addSearch.toLowerCase()) || (u.display_name_th || "").includes(addSearch))).map(u => (
                    <button key={u.key} className="w-full text-left px-3 py-2 hover:bg-muted text-sm flex items-center gap-2" onClick={() => { setAddUserId(u.key); setAddSearch(u.display_name_en || u.display_name_th || u.key) }}>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{u.key}</p>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="flex-1 overflow-auto space-y-2">
              ...
              {roleUsers.length === 0 && <p className="text-center text-sm text-muted-foreground py-4">{t("ROLES.NO_USERS")}</p>}
            </div>
          </DialogContent>
        </Dialog>
      )}
...
function PermCheckbox({ node, nameKey, selected, toggle, depth = 0 }) {
  ...
  return (
    <>
      <div className="flex items-center gap-1 py-1 px-1 rounded hover:bg-muted/50 text-sm" style={{ paddingLeft: depth * 20 }}>
        {hasChildren ? (
