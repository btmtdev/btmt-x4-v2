  ...
  return (
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex border-b border-border">
        <button onClick={() => setAuthTab("gateway")} className={`px-4 py-2 text-sm font-medium flex items-center gap-1.5 ${authTab === "gateway" ? "border-b-2 border-primary text-primary" : "text-muted-foreground"}`}>
          Gateway <span className="inline-flex items-center justify-center min-w-5 h-5 px-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold">{users.filter(u => u.auth_mode === "gateway" && !u.is_deleted).length}</span>
        </button>
        <button onClick={() => setAuthTab("local")} className={`px-4 py-2 text-sm font-medium flex items-center gap-1.5 ${authTab === "local" ? "border-b-2 border-primary text-primary" : "text-muted-foreground"}`}>
          Local <span className="inline-flex items-center justify-center min-w-5 h-5 px-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold">{users.filter(u => u.auth_mode === "local" && !u.is_deleted).length}</span>
        </button>
      </div>
      ...
      <Dialog open onOpenChange={v => !v && setShowDeleted(false)}>
        <DialogContent className="max-w-md max-h-[70vh] flex flex-col">
          ...
          <div className="flex-1 overflow-auto space-y-2">
            {deletedUsers.map(u => (
              <div key={u.key} className="flex items-center gap-3 border rounded-lg px-3 py-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{i18n.language === "th" ? (u.display_name_th || u.display_name_en || u.key) : (u.display_name_en || u.display_name_th || u.key)}</p>
                  <p className="text-sm text-muted-foreground truncate">{u.key}</p>
                </div>
            ...
            {deletedUsers.length === 0 && <p className="text-center text-sm text-muted-foreground py-4">{t("AG_GRID.NO_ROWS")}</p>}
          </div>
        </DialogContent>
      </Dialog>
    )}
