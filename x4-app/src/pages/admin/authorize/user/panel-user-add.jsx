    ...
    <Dialog open onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-md">
        ...
        <div className="flex border-b border-border">
          <button onClick={() => setAuthMode("gateway")} className={`flex-1 py-2 text-xs font-medium ${authMode === "gateway" ? "border-b-2 border-primary text-primary" : "text-muted-foreground"}`}>Gateway</button>
          <button onClick={() => setAuthMode("local")} className={`flex-1 py-2 text-xs font-medium ${authMode === "local" ? "border-b-2 border-primary text-primary" : "text-muted-foreground"}`}>Local</button>
        </div>
        <div className="space-y-4 pt-3">
          ...
          {authMode === "gateway" && (
            <div className="space-y-4">
              <div>
                <label className="text-xs font-medium text-muted-foreground">{t("USERS.LOOKUP_EMPLOYEE")}</label>
              ...
              {profile && (
                <div className="rounded-lg border border-border overflow-hidden">
                  <div className="bg-muted/50 px-3 py-2 flex items-center gap-2 border-b border-border">
                    ...
                    <div>
                      <p className="text-xs font-medium leading-tight">{profile.display_name_en || profile.display_name_th}</p>
                      <p className="text-xs text-muted-foreground">{profile.emp_code}</p>
                    </div>
                  </div>
                  ...
                  {profile.work_status && profile.work_status !== "Active" && (
                    <div className="px-3 py-2 bg-red-50 border-t border-border text-xs text-red-600">{t("USERS.NOT_ACTIVE_WARNING")}</div>
                  )}
                  {error && (
                    <div className="px-3 py-2 bg-red-50 border-t border-border text-xs text-red-600">{error}</div>
                  )}
                </div>
              )}
              {!profile && error && (
