...
export function PreferencesPanel({ userId, onClose }) {
  ...
  function applyFontSize(s) {
    ...
    document.documentElement.style.fontSize = (v / 16 * 100) + "%"
  }
      ...
      <div className="fixed top-(--header-height) bottom-0 right-0 z-[55] w-80 bg-background border-l border-border shadow-lg flex flex-col">
        ...
        <div className="flex-1 p-4 space-y-6 overflow-y-auto">
          <div className="space-y-2">
            <div>
              <label className="text-sm font-bold uppercase">{t("PREFS.PRIMARY_COLOR")}</label>
            ...
            <div className="flex items-center gap-1.5 pt-1">
              <span className="text-sm font-medium" style={{ color: color || undefined }}>#</span>
              <input
                type="text"
                value={(color || "").replace("#", "").toUpperCase()}
                onChange={e => { const v = "#" + e.target.value.replace(/[^0-9a-fA-F]/g, "").slice(0, 6).toUpperCase(); if (v.length === 7) applyColor(v) }}
                placeholder="HEX"
                className="w-16 text-sm px-1.5 py-1 border rounded uppercase font-mono"
              ...
              <button
                ...
                className="size-5 rounded-sm border border-dashed border-muted-foreground flex items-center justify-center text-muted-foreground text-sm hover:border-primary hover:text-primary"
              >+</button>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div>
              <label className="text-sm font-bold uppercase">{t("PREFS.FONT_SIZE")}</label>
              <Separator className="mt-1 mb-2" />
            </div>
          ...
          <div className="space-y-2">
            <div>
              <label className="text-sm font-bold uppercase">{t("PREFS.DEFAULT_LANGUAGE")}</label>
              <Separator className="mt-1 mb-2" />
            </div>
