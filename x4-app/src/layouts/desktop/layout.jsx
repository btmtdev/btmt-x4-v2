...
import { getEnvName } from "@/lib/env"
  ...
  return (
    <LayoutContext.Provider value={{ isMobile, isPanelOpen, panelToggle, isMobileOpen, mobileToggle, style }}>
      <TooltipProvider delayDuration={0}>
        <div data-slot="layout-wrapper" className="flex grow bg-muted/40" style={style}>
          {getEnvName() !== "production" && <div className="fixed top-0 left-0 z-[99] overflow-hidden size-16 pointer-events-none"><div className={`absolute top-[6px] -left-[22px] w-[90px] text-center rotate-[-45deg] text-white text-[10px] font-bold py-0.5 shadow-sm ${getEnvName() === "development" ? "bg-amber-500" : "bg-blue-500"}`}>{getEnvName() === "development" ? "DEV" : "UAT"}</div></div>}
          {children}
        </div>
      </TooltipProvider>
...
function Breadcrumb({ menu, nameKey, toolbar }) {
  ...
  return (
    <div className="px-5 h-10 flex items-center gap-1 text-sm text-muted-foreground shrink-0">
      {hasPanel && <button onClick={panelToggle} className="hidden lg:flex items-center justify-center size-7 rounded-md hover:bg-accent me-1">
        <Menu size={15} />
      </button>}
      {crumbs.map((c, i) => (
        <Fragment key={c.id}>
...
function LayoutInner({ menu, nameKey, toolbar }) {
  ...
  return (
    <>
      ...
      {outdated && (
        <div className="fixed bottom-0 inset-x-0 z-50 bg-yellow-300 border-t border-yellow-400 px-4 py-1.5 flex items-center justify-center gap-2 text-sm text-yellow-900">
          <RefreshCw size={14} />
...
export default function DesktopLayout() {
  ...
  useEffect(() => {
    ...
    adminService.getUserSettings(user.key).then(prefs => {
      ...
      if (prefs.font_size) document.documentElement.style.fontSize = (Number(prefs.font_size) / 16 * 100) + "%"
